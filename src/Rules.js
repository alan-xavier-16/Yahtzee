...
class FullHouse extends Rule {
    evalRoll = dice => {
        // Must have 2 dies with same value and 3 dies with same value e.g. [2, 2, 3, 3, 3]
...
class SmallStraight extends Rule {
    evalRoll = dice => {
        ...
        // small straight has 4+ values in a row, e.g. 1234,  2345,  3456
        return d.size >= 4 && !( (d.has(1) && d.has(6)) && d.has(1, 6) && d.has(1, 2) && d.has(5, 6) ) ? this.score : 0;
    }
}
...
const fullHouse = new FullHouse({ score: 25, description: "25 pts for a full house" });

// small/large straights score as 30/40
const smallStraight = new SmallStraight({ score: 30, description: "30 pts for a small straight" });
const largeStraight = new LargeStraight({ score: 40, description: "40 pts for a large straight" });
